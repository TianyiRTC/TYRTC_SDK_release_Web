...
<%@ page import="ctbri.Ctrtc" %>
<%
String path = request.getContextPath();
String basePath = request.getScheme()+"://"+request.getServerName()+":"+request.getServerPort()+path+"/";
%>
<%
	String appaccount = request.getParameter("user");
	String terminalType = request.getParameter("terminal");
	if(appaccount==null){appaccount = String.valueOf(System.currentTimeMillis());}
	if(terminalType==null){terminalType = "Browser";}
	String appid = "changeit";
	String appkey = "changeit";
	Ctrtc ctrtc = Ctrtc.getInstance(appid, appkey);
	String token = ctrtc.generateToken(ctrtc.ACC_INNER, appaccount,terminalType);
 %>
...
<html>
  <head>
	...
	<script src="<%=basePath%>js/ctrtc.min.js" type="text/javascript"></script><!-- 引用jssdk -->
...
<body>
		应用ID[<%=appid%>]用户[<%=appaccount%>]终端类型[<%=terminalType%>]</><br>
		<button onclick="call();">
			呼叫
		</button>
    <button onclick="hangup();">
      全部挂断
		...
		<script type="text/javascript">
			...
			Ctrtc.Device.init("<%=token%>",{dbg:true,defaultResolution:'cif'});
